...
class Account(models.Model):
    ...
    code = models.CharField(max_length=20, unique=True, help_text="Account code (e.g., 1010, 5010)")
...
class TaxType(models.Model):
    ...
    code = models.CharField(max_length=20, unique=True, help_text="Tax code (e.g., IVAT, OVAT, EWT2)")
    name = models.CharField(max_length=100, help_text="Tax name (e.g., Input VAT, Output VAT, EWT 2%)")
    description = models.TextField(blank=True)
...
class JournalHeader(models.Model):
    ...
    journal_number = models.CharField(max_length=50, unique=True, help_text="Journal entry number")
    transaction_date = models.DateField(help_text="Transaction date")
    reference = models.CharField(max_length=100, blank=True, help_text="External reference (invoice, check number, etc.)")
    description = models.TextField(help_text="Journal entry description")
    status = models.CharField(max_length=10, choices=STATUS_CHOICES, default='DRAFT')
    
    created_by = models.ForeignKey(User, on_delete=models.PROTECT, related_name='journals_created')
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
...
class JournalLine(models.Model):
    ...
    description = models.CharField(max_length=200, blank=True, help_text="Line-level description")
